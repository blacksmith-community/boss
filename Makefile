...
.PHONY: build
build: ## Build the boss binary for current OS/architecture
	@echo "$(GREEN)Building $(BINARY_NAME)...$(RESET)"
	@go build -ldflags="$(LDFLAGS)" -o $(BINARY_NAME) .
	@echo "$(GREEN)✓ Build complete$(RESET)"

.PHONY: linux
linux: ## Build the boss binary for Linux AMD64
...
shipit: ## Build release artifacts (requires VERSION env var)
	...
	@echo "$(GREEN)Compiling boss Broker binaries...$(RESET)"
	@rm -rf artifacts
	@mkdir artifacts
	...
	@echo "$(GREEN)Assembling Server Distribution with platform binaries...$(RESET)"
	@rm -f artifacts/*.tar.gz artifacts/*.tar.bz2
	@rm -rf boss-$(VERSION)
	@mkdir -p boss-$(VERSION)
